...
module ScrollingText(
    ...
    // Make sure text is written only after write switch goes low after being high
    logic can_write = 1;
    ...
    logic scroll_clk;
    ClockDivider #(10) slow(.clk(clk), .div_clk(scroll_clk));
    ...
    always @(posedge debounce_clk, posedge reset) begin
        ...
        if (reset == 1) begin
            ...
            case (ps)
                st_write:
                begin
                    if (~start & write & can_write) begin // New char entered
                        can_write = 0; // Wait for write switch to go low before writing again
                        ...
                        */
                        ...
                        // If write switch goes low, allow for more writing
                        can_write = 1;
                        ns = st_write;
                    end
